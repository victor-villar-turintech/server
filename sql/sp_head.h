...
public:
  ...
  */
  ...
  /**
    ...
    It is checked separately by the callers, e.g.
    - LEX::sp_body_finalize_trigger(): a trigger body is never executed in
      a PS safe context, so dynamic SQL is rejected at CREATE TRIGGER time.
    - Sql_cmd_call::execute() and sp_package::instantiate_if_needed():
      for a routine used from a trigger or from a stored function.
  */
